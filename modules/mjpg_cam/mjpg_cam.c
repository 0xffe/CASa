...
int main(int argc, char *argv[])
{
	...
	//openlog("MJPG_Cam", LOG_PID | LOG_CONS | LOG_PERROR, LOG_USER);
	openlog("MJPG_Cam ", LOG_PID | LOG_CONS, LOG_USER);
	...
	if (global.incnt == 0)
		input[global.incnt++] = strdup(DEFAULT_INPUT_MODE);
		
	if (global.outcnt == 0)
		output[global.outcnt++] = strdup(DEFAULT_OUTPUT_MODE);
     ...
     */
#ifdef SVN_REV
    LOG("MJPG_Cam Version: svn rev: %s\n", SVN_REV);
#else
    LOG("MJPG_Cam Version.: %s\n", SOURCE_VERSION);
#endif
